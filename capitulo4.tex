...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[
        ...
        \node[purple] at (2.0, \H+0.4, 0.2) {Flujo huecos ($I$)};

        % --- CUERPO DEL SEMICONDUCTOR (Caras frontales y superior) ---
        \fill[cyan!20, opacity=0.4, draw=brown!80!black, thick] (0,0,\D) -- (\W,0,\D) -- (\W,\H,\D) -- (0,\H,\D) -- cycle; % Frontal
        ...
        \draw[<->, red, ultra thick] (\W+0.4, \H/2, \D) -- (\W+0.4, \H/2, 0) 
            node[midway, right, yshift=-3pt] {$V_H$};
        \node[red, align=left] at (\W+0.2, \H-1.5, \D/2) {Tensión\\transversal};

        % --- DIMENSIONES ---
        % Líneas de referencia y cota para d (espesor)
        \draw[gray, thin] (0, 0, \D) -- (-0.5, 0, \D);
        \draw[gray, thin] (0, \H, \D) -- (-0.5, \H, \D);
        \draw[<->, gray] (-0.4, 0, \D) -- (-0.4, \H, \D) node[midway, left] {$d$};
...
Los \emph{sensores GMR} (Giant Magnetoresistance, Nobel de Física 2007) alcanzan variaciones de hasta el \qty{80}{\%} mediante multicapas cuánticas ferromagnético/antiferromagnético (Fe/Cr). Son la base de los cabezales de lectura de discos duros y de los sensores de ángulo en los codificadores magnéticos de precisión. En la Tabla~\ref{tab:hall_vs_mr} se comparan las características de los sensores Hall, AMR y GMR para aplicaciones de instrumentación magnética. 
...
\paragraph{Puente de Maxwell} Se utiliza para medir una inductancia desconocida ($L_x, R_x$) comparándola con un condensador patrón ($C_s$). Su gran ventaja es que las condiciones de equilibrio son independientes de la frecuencia.
...
En aplicaciones donde la señal debe transmitirse a larga distancia o en entornos con mucho ruido de amplitud, se prefiere convertir la variación de $L$ o $C$ en una variación de \textbf{frecuencia}. 

Al integrar el sensor en un circuito resonante $LC$, la frecuencia de oscilación viene dada por la fórmula de Thomson:
\begin{equation}
    f_r = \frac{1}{2\pi \sqrt{LC}}
\end{equation}
Cualquier cambio en la magnitud física que altere la capacidad del sensor provocará un desplazamiento de la frecuencia ($\Delta f$). Esta señal digital es intrínsecamente robusta y fácil de medir mediante el contador/temporizador de un microcontrolador sin necesidad de un ADC de alta precisión.
...
\end{equation}
Tras pasar por un filtro paso bajo (LPF), eliminamos el término de alta frecuencia ($2\omega$), obteniendo una tensión de continua proporcional al coseno del desfase:
\begin{equation}
    V_{out, DC} = \frac{V_s V_r}{2} \cos(\phi)
\end{equation}

\marginnote{\textbf{Detección de dirección:} En el LVDT, si el núcleo se mueve a la derecha, $\phi=0^\circ \implies \cos(0)=1$ (V positiva). Si se mueve a la izquierda, $\phi=180^\circ \implies \cos(180)=-1$ (V negativa). Así recuperamos el signo del desplazamiento.}
...
\begin{table}[!htbp]
    \centering
    \caption[Comparativa general de tecnologías reactivas.]{Comparativa general de tecnologías reactivas.}\label{tab:comparativa_final}
    \small
