...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \node[font=\tiny, anchor=west] at (axis cs:3.15, 3.175) {$e_i$};
        \end{axis}
    \end{tikzpicture}
...
Para obtener la mejor representación lineal de los datos obtenidos en el laboratorio se aplica el \emph{Método de los Mínimos Cuadrados}.\marginnote{\textbf{Método de los Mínimos Cuadrados ($m$ y $b$):} las fórmulas de las Ecuaciones~\eqref{eq:m_final} y~\eqref{eq:b_final} se obtienen minimizando la función de error cuadrático de la Ecuación~\eqref{eq:error_cuadratico} --la suma de los cuadrados de los residuos-- e igualando a cero sus derivadas parciales respecto de $m$ y de $b$, tal como se desarrolla a continuación. Ese punto es el mínimo de la función y corresponde a la recta de mejor ajuste de los datos experimentales.}
...
Reorganizando los términos se obtienen las \emph{ecuaciones normales}, Ecuaciones~\eqref{eq:normal1} y~\eqref{eq:normal2}:
...
Despejando $b$ de la Ecuación~\eqref{eq:normal2}:
\[ b = \frac{\sum y_i - m\sum x_i}{n} = \bar{y} - m\bar{x} \]
y sustituyendo ese valor en la Ecuación~\eqref{eq:normal1} se obtiene la pendiente. Este método garantiza que la recta de ajuste $y = mx + b$ minimice la suma de los cuadrados de las distancias verticales (residuos) de cada punto experimental a la recta. Las fórmulas para calcular la pendiente ($m$) y la ordenada en el origen ($b$) son:
...
\begin{equation}\label{eq:error_no_linealidad}
...
\begin{marginfigure}
    \centering
    \begin{tikzpicture}[x={\linewidth/2.8}, y={\linewidth/2.8}]
      \shorthandoff{>}
        ...
        \node[blue, anchor=north west] at (1.6,0.55) {asc.};
        \node[red, anchor=south east] at (0.6,1.85) {desc.};
    \end{tikzpicture}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[x={\linewidth/2.8}, y={\linewidth/2.8}]
      \shorthandoff{>}
        ...
        \node[red, anchor=east, align=right] at (0.9,1.1) {Banda de\\Muerta};
    \end{tikzpicture}
...
La \emph{repetibilidad} es la capacidad de un sensor para entregar la misma salida al aplicar la misma entrada varias veces en periodos cortos de tiempo y en las mismas condiciones de prueba --mismo operador, mismo instrumento, mismo laboratorio--. Cuando lo que cambia es alguna de esas condiciones --otro operador, otro instrumento, otro laboratorio o, simplemente, otro día-- deja de hablarse de repetibilidad y se habla de \emph{reproducibilidad}, que es siempre peor que la repetibilidad del propio instrumento y es la cifra que importa cuando la medida la va a repetir otra persona (Capítulo~\ref{cap:metrologia}). Como la dispersión de la repetibilidad es un error aleatorio, no se corrige por calibración: solo se reduce promediando lecturas o filtrando. Un sensor con alta repetibilidad tendrá una desviación estándar baja en sus mediciones, lo que indica que las salidas son consistentes y confiables. La repetibilidad se evalúa realizando varias mediciones con el mismo valor de entrada y analizando la dispersión de las salidas obtenidas: la Tabla~\ref{tab:repetibilidad} recoge cinco lecturas de la misma presión. En la Figura~\ref{fig:repetibilidad} se muestra un ejemplo gráfico de repetibilidad, donde se realizan varias mediciones con el mismo valor de entrada. La dispersión de los puntos alrededor de la media indica el nivel de repetibilidad del sensor, siendo una dispersión menor indicativa de una mejor repetibilidad.\marginnote{\textbf{Ejemplo de repetibilidad.} Un ejemplo típico de repetibilidad es el comportamiento de un sensor de temperatura, donde se espera que el sensor entregue la misma salida para la misma temperatura en múltiples mediciones realizadas en periodos cortos de tiempo. Otro ejemplo típico de repetibilidad es el comportamiento de un sensor de presión, donde se espera que el sensor entregue la misma salida para la misma presión en múltiples mediciones realizadas en periodos cortos de tiempo.} 

\begin{margintable}
    \centering
    \caption[Cinco lecturas de la misma presión para evaluar la repetibilidad.]{Cinco lecturas consecutivas de la misma presión, con el sensor trabajando sobre un fondo de escala de \SI{0}{\volt} a \SI{5}{\volt}. \protect\label{tab:repetibilidad}}
    \begin{tabular}{c c}
        \toprule
        Medición & Salida (V) \\
        \midrule
...
De las cinco lecturas se obtiene una media de \SI{2.0000}{\volt} y una desviación estándar de \SI{0.95}{\milli\volt}, de modo que el \emph{índice de repetibilidad} $r$ --que resume el efecto del ruido y de la deriva a corto plazo-- es el doble de esa desviación:
\begin{equation}\label{eq:repetibilidad}
    \bar{y} = \SI{2.0000}{\volt}, \qquad s = \sqrt{\frac{\sum_{i=1}^{n}{(y_i - \bar{y})}^2}{n-1}} = \SI{0.95}{\milli\volt}, \qquad r = 2s = \SI{1.9}{\milli\volt}
\end{equation}
El índice se declara casi siempre en porcentaje del fondo de escala --\SI{1.9}{\milli\volt} sobre \SI{5}{\volt} son el \SI{0.04}{\percent}--, aunque también puede referirse a la lectura, en cuyo caso el mismo error absoluto pesa más cuanto menor es la lectura: sobre los \SI{2}{\volt} medidos representa el \SI{0.1}{\percent}.
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            xlabel={Medición}, ylabel={Salida (V)},
            font=\scriptsize,
            axis lines=left,
            xmin=0.5, xmax=5.5, ymin=1.995, ymax=2.005,
            xtick={1,2,3,4,5},
            ytick={1.995,2.000,2.005},
            yticklabels={$1{,}995$,$2{,}000$,$2{,}005$},
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
        ]
            % Media (recta de referencia)
            \addplot[gray, dashed, thick, domain=0.5:5.5] {2.000};
            \node[gray, font=\tiny, anchor=south west] at (axis cs:0.55,2.0005) {media};
            % Cinco lecturas consecutivas (Tabla 2.2)
            \addplot[only marks, mark=*, blue, mark size=1.3pt] coordinates {
                (1, 2.0000) (2, 2.0012) (3, 1.9994) (4, 2.0006) (5, 1.9988)
            };
        \end{axis}
    \end{tikzpicture}
    \caption[Ejemplo de repetibilidad.]{Ejemplo de repetibilidad. Un sensor con buena repetibilidad producirá resultados consistentes bajo las mismas condiciones de prueba: las cinco lecturas de la Tabla~\ref{tab:repetibilidad} se agrupan estrechamente en torno a la media.}\label{fig:repetibilidad}
\end{marginfigure}
...
\begin{equation}\label{eq:error_histeresis}    
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \node[blue, rotate=43] at (axis cs:1.05,1.85) {Zona Lineal};
            \node[red] at (axis cs:4.2,3.35) {Saturación};
        \end{axis}
    \end{tikzpicture}
...
Los defectos de la curva de calibración descritos en esta sección --cero, span, no linealidad, histéresis y repetibilidad-- son los que el certificado de calibración de un instrumento declara por separado, junto con la banda muerta y el efecto de carga, que dependen de cómo se instala el sensor. La Tabla~\ref{tab:errores_estaticos} reúne su expresión cuantitativa y la forma de combatirlos.

\begin{table}[ht]
    \centering
    \small
    \setlength{\tabcolsep}{3pt}
    \begin{tabularx}{\linewidth}{@{} L{0.22\linewidth} >{\centering\arraybackslash}X >{\centering\arraybackslash}X @{}}
        \toprule
        \textbf{Error} & \textbf{Expresión cuantitativa} & \textbf{Cómo se combate y se declara} \\
        \midrule
        Error de cero ($e_0$) & $e_0 = x_{ind} - x$, con la pendiente ya corregida & Ajuste de cero contra un patrón; se declara en unidades absolutas (Ecuación~\eqref{eq:error_cero_span}). \\
        Error de span ($\epsilon_s$) & $\epsilon_s = \frac{m - m_{nom}}{m_{nom}}$ & Ajuste de span o de ganancia; se declara en \% de la lectura (Ecuación~\eqref{eq:error_cero_span}). \\
        No linealidad ($\epsilon_{lin}$) & $\frac{|y_i - (mx_i+b)|_{\max}}{y_{\max}-y_{\min}}\times 100$\,\% & Un ajuste de dos puntos no la corrige: exige un polinomio de linealización o una tabla de corrección (Ecuación~\eqref{eq:error_no_linealidad}). \\
        Histéresis ($\epsilon_{his}$) & $\frac{|y_{asc}-y_{desc}|_{\max}}{y_{\max}-y_{\min}}\times 100$\,\% & Es un defecto mecánico o magnético del propio sensor: se combate en el diseño, no en el software (Ecuación~\eqref{eq:error_histeresis}). \\
        Repetibilidad ($r$) & $r = 2s$ sobre $n$ medidas con la misma entrada & La degradan el ruido y la deriva a corto plazo; se declara en \% del fondo de escala (Ecuación~\eqref{eq:repetibilidad}). \\
        Banda muerta & Entrada mínima que produce un cambio apreciable en la salida & La fijan la fricción o el umbral de disparo: conviene trabajar con entradas muy por encima de ella (Figura~\ref{fig:banda_muerta}). \\
        Efecto de carga & $\frac{\Delta v}{v_{tx}} \approx -\frac{R_{out}}{R_{in}}$ & Se combate con una etapa de entrada de alta impedancia (Ecuación~\eqref{eq:error_carga}). \\
        \bottomrule
    \end{tabularx}
    \caption[Errores estáticos de un sistema de medida.][3cm]{Errores estáticos de un sistema de medida, su expresión cuantitativa y la forma de combatirlos y declararlos. Los cinco primeros son defectos de la curva de calibración y aparecen separados en el certificado del instrumento; los dos últimos dependen de la instalación y del campo de medida. \protect\label{tab:errores_estaticos}}
\end{table}
...
\[ y(t) = K \cdot x(t) \]
donde $K = b_0/a_0$ es una constante de ganancia: la sensibilidad estática del sistema.
...
Corresponde a sensores con almacenamiento de energía (ej.\ una capacidad térmica de un termómetro o un condensador en un circuito). En este caso la salida $y(t)$ depende de la entrada $x(t)$ y de la constante de tiempo $\tau$. Matemáticamente, se modela mediante una ecuación diferencial lineal de primer orden:
\[ \tau \frac{dy(t)}{dt} + y(t) = K \cdot x(t) \]
donde $K = b_0/a_0$ es una constante de ganancia: la sensibilidad estática del sistema. Si la entrada es un escalón de amplitud $A$ --es decir, $x(t) = A$ a partir de $t = 0$--, la solución de esa ecuación diferencial es
\begin{equation}\label{eq:primer_orden}
    y(t) = K A \left( 1 - e^{-t/\tau} \right)
\end{equation}
de modo que la salida se aproxima exponencialmente al valor final $K\,A$, como muestra la Figura~\ref{fig:primer_orden}. La constante de tiempo $\tau$ es el parámetro que define la rapidez del sensor: un valor de $\tau$ más pequeño indica una respuesta más rápida, mientras que un valor de $\tau$ más grande indica una respuesta más lenta. Las cifras que conviene recordar se deducen directamente de la Ecuación~\eqref{eq:primer_orden}: la salida alcanza el \SI{63.2}{\percent} del valor final en $t = \tau$, el \SI{95}{\percent} en $t = 3\tau$ y el \SI{99.3}{\percent} en $t = 5\tau$, de donde el tiempo de establecimiento al \SI{5}{\percent} de un sistema de primer orden se estima en $t_s \approx 3\tau$.\marginnote{\textbf{Constante de Tiempo ($\tau$):} Es el parámetro que define la rapidez del sensor. Representa el tiempo necesario para que la salida alcance el \qty{63.2}{\percent} de su valor final ante un escalón.
}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption[Respuesta al escalón de un sistema de primer orden.]{Respuesta al escalón de un sistema de primer orden, gobernada por la Ecuación~\eqref{eq:primer_orden}. La curva azul representa la salida $y(t)$, que se aproxima a su valor final $K\,A$ de manera exponencial, y las líneas punteadas indican el tiempo $\tau$ necesario para que la salida alcance el \SI{63.2}{\percent} de su valor final.}\label{fig:primer_orden}
\end{marginfigure}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            xtick=\empty,
            ytick={1, 1.37}, yticklabels={$K$, $y_{max}$},
            ...
            \draw[dotted, gray] (axis cs:0,1.37) -- (axis cs:1.1,1.37);
            \node[anchor=south east, blue, font=\tiny] at (axis cs:0.5, 1.37) {$M_p$};
...
Los tres modelos anteriores --orden cero, uno y dos-- describen la práctica totalidad de los sensores industriales. La Tabla~\ref{tab:ordenes_sistemas} reúne su ecuación, los parámetros que los caracterizan y el aspecto de su respuesta al escalón.

\begin{table}[ht]
    \centering
    \small
    \setlength{\tabcolsep}{3pt}
    \begin{tabularx}{\linewidth}{@{} >{\centering\arraybackslash}m{1.1cm} L{0.26\linewidth} >{\centering\arraybackslash}X >{\centering\arraybackslash}X @{}}
        \toprule
        \textbf{Orden} & \textbf{Ecuación que lo gobierna} & \textbf{Parámetros característicos} & \textbf{Respuesta al escalón y ejemplos} \\
        \midrule
        Cero & $y(t) = K\,x(t)$ & $K$ (sensibilidad estática) & Réplica instantánea de la entrada, sin retardo ni error dinámico (Figura~\ref{fig:orden_0}). Divisores de tensión y potenciómetros. \\
        Uno & $\tau \frac{dy}{dt} + y = K\,x(t)$ & $\tau$ (constante de tiempo) y $K$ & Exponencial: \SI{63.2}{\percent} del valor final en $t = \tau$ y $t_s$ al \SI{5}{\percent} en $t \approx 3\tau$ (Figura~\ref{fig:primer_orden}). Termómetros encapsulados, RTD en vaina y termopares con funda. \\
        Dos & $\frac{d^2y}{dt^2} + 2\zeta\omega_n\frac{dy}{dt} + \omega_n^2 y = \omega_n^2 K\,x(t)$ & $\omega_n$ (frecuencia natural), $\zeta$ (amortiguamiento) y $K$ & Oscilatoria si $\zeta < 1$, con $M_p = e^{-\pi\zeta/\sqrt{1-\zeta^2}}$ (Figuras~\ref{fig:segundo_orden} y~\ref{fig:ejemplo_segundo_orden}). Acelerómetros piezoeléctricos y sensores de presión de diafragma. \\
        \bottomrule
    \end{tabularx}
    \caption[Los tres modelos dinámicos de un sensor.][3cm]{Los tres modelos dinámicos de un sensor: ecuación, parámetros característicos y respuesta al escalón, con los ejemplos más frecuentes de cada uno. La constante de tiempo $\tau$ fija la rapidez del sistema de primer orden; en el de segundo orden, la rapidez la fija la frecuencia natural $\omega_n$ y la estabilidad, el amortiguamiento $\zeta$. \protect\label{tab:ordenes_sistemas}}
\end{table}
...
\begin{itemize}
    \item \emph{Tiempo de subida} ($t_r$): lo que tarda la salida en pasar del \SI{10}{\percent} al \SI{90}{\percent} de su valor final. Depende de los dos parámetros --crece con el amortiguamiento y se acorta al subir la frecuencia natural-- y vale entre $1{,}2/\omega_n$ y $2{,}1/\omega_n$ para amortiguamientos entre $0{,}2$ y $0{,}7$ (con el $\zeta = 0{,}35$ de la Figura~\ref{fig:prestaciones_dinamicas}, $t_r \approx 1{,}4/\omega_n$): la frecuencia natural fija la escala de rapidez del sensor.
...
En un sistema de medida digital hay que distinguir entre el límite teórico que impone el hardware y el límite práctico que impone la física del ruido. La \emph{resolución nominal} la fija el diseño del convertidor analógico a digital (ADC): si el rango de entrada es $FS$ (\textit{Full Scale}) y el convertidor tiene $n$ bits, el paso de cuantificación o \emph{escalón} $\Delta$ vale
\begin{equation}\label{eq:resolucion_nominal}
    \Delta = \frac{FS}{2^n}
\end{equation}
y es el cambio más pequeño que el convertidor puede representar en ausencia total de ruido; los detalles de la cuantificación se estudian en el Capítulo~\ref{cap:daq} (Sección~\ref{sec:cuantificacion}). La \emph{resolución real} o \emph{efectiva} es la que el sistema alcanza en la práctica y depende de la relación entre el ruido presente en la señal, expresado por su valor eficaz (\textit{Root Mean Square}, RMS) $V_{n,RMS}$, y ese escalón. El ruido eficaz no es más que el suelo de ruido de la Ecuación~\eqref{eq:noise_floor}, $V_{n,RMS} = \sqrt{4kTR\,\Delta f}$, con $k$ la constante de Boltzmann, $T$ la temperatura absoluta, $R$ la resistencia equivalente del sistema y $\Delta f$ su ancho de banda, de modo que $V_{n,RMS}$ representa el nivel mínimo de señal que el sistema puede detectar por encima de su propio ruido.\marginnote{\textbf{Ruido \emph{pico a pico}:} Para que una medida sea estable en un display, el ruido pico a pico ($V_{pp}$) debe ser menor que $\Delta$. En una distribución gaussiana, se estima que $V_{pp} \approx 6.6 \cdot V_{n,RMS}$. Si $V_{pp} > \Delta$, los bits menos significativos oscilarán aleatoriamente, un fenómeno conocido como \textit{flicker} o parpadeo.} Ese modelo de ruido es solo el térmico (o de Johnson-Nyquist): el ruido de disparo, el de \textit{flicker} y los acoplamientos externos se suman a él, por lo que el valor real de $V_{n,RMS}$ es siempre mayor que el que predice la ecuación (Capítulo~\ref{cap:ruido}).

En la Figura~\ref{fig:resolucion_real_vs_nominal} se representa el efecto de ese escalón sobre una señal: la salida digital solo puede tomar múltiplos de $\Delta$ y sustituye la curva analógica por una escalera.
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            width=\linewidth, height=5cm,
            xlabel={$t$}, ylabel={$y$},
            xmin=0, xmax=10, ymin=0.15, ymax=1.4,
            axis lines=left, font=\scriptsize,
            trig format plots=rad,
            xtick=\empty,
            ...
            \draw[gray!40, dashed] (axis cs:0,0.25) -- (axis cs:10,0.25);
            \draw[gray!40, dashed] (axis cs:0,0.5) -- (axis cs:10,0.5);
            \draw[gray!40, dashed] (axis cs:0,0.75) -- (axis cs:10,0.75);
            \draw[gray!40, dashed] (axis cs:0,1) -- (axis cs:10,1);
            \draw[gray!40, dashed] (axis cs:0,1.25) -- (axis cs:10,1.25);

            % Señal analógica
            \addplot[thick, blue, domain=0:10, samples=200]
                {0.7 + 0.5*sin(0.6*x) + 0.01*sin(9*x)};

            % Salida cuantificada (Δ = 0.25)
            \addplot[thick, red!75!black, domain=0:10, samples=800]
                {0.25*floor((0.7 + 0.5*sin(0.6*x) + 0.01*sin(9*x))/0.25 + 0.5)};

            % Escalón de cuantificación
            \draw[<->, thick, gray] (axis cs:2.6,1) -- (axis cs:2.6,1.25);
            \node[gray, font=\tiny, anchor=west] at (axis cs:2.7,1.1) {$\Delta$};
        \end{axis}
    \end{tikzpicture}
    \caption[Resolución nominal frente a resolución real.]{Resolución nominal frente a resolución real. La curva azul es la señal analógica y la escalera roja, la salida del convertidor, que solo puede tomar múltiplos del escalón $\Delta = FS/2^n$. Si el ruido eficaz es mucho menor que $\Delta$, como en la figura, la resolución real coincide con la nominal; si lo supera, los bits menos significativos fluctúan y la resolución efectiva se degrada por debajo de la nominal.}\label{fig:resolucion_real_vs_nominal}
\end{marginfigure}

El comportamiento del sistema se clasifica, por tanto, en dos regímenes:
\begin{enumerate}
    \item \emph{Sistema limitado por cuantificación} ($V_{n,RMS} \ll \Delta$): el ruido es tan bajo que queda \emph{escondido} dentro de un escalón de cuantificación. La resolución real coincide con la nominal y los bits menos significativos (\textit{Least Significant Bits}, LSB) son estables y contienen información útil de la magnitud física.
    \item \emph{Sistema limitado por ruido} ($V_{n,RMS} > \Delta$): el ruido supera el escalón de cuantificación, la resolución real se degrada y el número efectivo de bits (ENOB, \emph{Effective Number of Bits}) queda por debajo del nominal ($n$). Los bits afectados por el ruido fluctúan aleatoriamente y no aportan información válida sobre la medida.
...
El ENOB cuantifica esa pérdida de bits a partir del SINAD del convertidor, y se calcula como se detalla en la Sección~\ref{sec:enob} del Capítulo~\ref{cap:daq}: la conclusión práctica es que la resolución que hay que comparar con los requisitos de la aplicación no es la del \textit{datasheet} del ADC, sino la que deja el ruido de toda la cadena de medida.
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\linewidth}{@{} L{0.24\linewidth} >{\centering\arraybackslash}X >{\centering\arraybackslash}X @{}}
        ...
        Histéresis, repetibilidad y reproducibilidad & Diferencia máxima entre las ramas, dispersión con la misma entrada y dispersión al cambiar de operador, equipo o laboratorio, en \% del fondo de escala & Sección~\ref{sec:histeresis_repetibilidad} \\
        Banda muerta & Entrada mínima que produce un cambio apreciable en la salida & Sección~\ref{sec:histeresis_repetibilidad} \\
        ...
        Modelo dinámico (orden 0, 1 y 2) & Orden de la ecuación diferencial y sus parámetros: $\tau$, o $\omega_n$ y $\zeta$, con la ganancia $K$ & Sección~\ref{sec:caract_dinamicas} y Tabla~\ref{tab:ordenes_sistemas} \\
...
\begin{itemize}
    \item \emph{Características estáticas} (Sección~\ref{sec:caract_estaticas}): la curva de calibración y su sensibilidad, la linealidad y la recta de referencia, la histéresis, la repetibilidad y la reproducibilidad, la banda muerta, el rango y la saturación, y los dos defectos de la recta --error de cero y error de span (Figura~\ref{fig:errores_cero_span})--, junto con el efecto de carga, que aparece siempre que dos etapas con impedancias comparables se conectan entre sí. La Tabla~\ref{tab:errores_estaticos} los reúne todos con su expresión cuantitativa y la forma de combatirlos.
    \item \emph{Características dinámicas} (Sección~\ref{sec:caract_dinamicas}): la respuesta al escalón de los sistemas de orden cero, primero y segundo, con la constante de tiempo, la frecuencia natural y el amortiguamiento como parámetros; y las prestaciones con las que se especifican --tiempo de subida, sobreoscilación máxima, tiempo de establecimiento y ancho de banda (Figura~\ref{fig:prestaciones_dinamicas})--, con $\zeta \approx 0{,}7$ como el compromiso recomendado entre rapidez y estabilidad. La Tabla~\ref{tab:ordenes_sistemas} resume la ecuación, los parámetros y la respuesta de cada uno de los tres modelos.
    \item \emph{Límites de la medida} (Sección~\ref{sec:limites_medida}): el suelo de ruido, el rango dinámico y la relación señal-ruido (Figura~\ref{fig:noise_floor}), junto con la diferencia entre resolución nominal y resolución real (Ecuación~\eqref{eq:resolucion_nominal} y Figura~\ref{fig:resolucion_real_vs_nominal}), que es lo que fija hasta dónde se puede medir y no solo qué se puede medir.
