...
\begin{marginfigure}
  \begin{tikzpicture}[>=Stealth, scale=0.75,
    ...
    \begin{scope}
      ...
      \node[anchor=north west, font=\itshape, gray]
        at (-3.6, 1.35) {(a) $f_s > 2f_{max}$: sin aliasing};

      \draw[->] (-3.8,0) -- (3.8,0) node[right,font=\tiny] {$f$};
      \draw[->] (0,-0.1) -- (0,1.5) node[above,font=\tiny] {$|X(f)|$};
      ...
      % Etiquetas
      \node[font=\tiny, below] at ( 1, 0) {$f_{max}$};
      \node[font=\tiny, below] at (-1, 0) {$-f_{max}$};
      \node[font=\tiny, below] at ( 3, 0) {$f_s$};
      \node[font=\tiny, below] at (-3, 0) {$-f_s$};
    ...
    \begin{scope}[yshift=-3.0cm]
      ...
      \node[anchor=north west, font=\itshape, gray]
        at (-3.6, 1.35) {(b) $f_s < 2f_{max}$: aliasing};
      ...
      \fill[red!20, opacity=0.6]
        plot[domain=0.6:1.8, samples=40]
          (\x, {max(tri(\x), tri((\x-1.8)/1.2))})
        -- (1.8,0) -- (0.6,0) -- cycle;
      \fill[red!20, opacity=0.6]
        plot[domain=-1.8:-0.6, samples=40]
          (\x, {max(tri(\x), tri((\x+1.8)/1.2))})
        -- (-0.6,0) -- (-1.8,0) -- cycle;
      ...
      \node[font=\tiny, below] at ( 1.2, 0) {$f_{max}$};
      \node[font=\tiny, below] at ( 1.8, 0) {$f_s$};
...
\begin{marginfigure}
  \begin{tikzpicture}[>=Stealth, scale=1,
    ...
    \begin{scope}
      ...
      \node[anchor=north west, font=\tiny\itshape, gray]
        at (0, 2.45) {(a) $f_s = 4\,\text{Hz} > 2f_{max}$};

      \draw[->] (0,0) -- (5.6,0) node[right,font=\tiny] {$t$};
      \draw[->] (0,-1.35) -- (0,1.55) node[above,font=\tiny] {$x$};
...
\draw[blue!70!black, thick]
  plot[domain=0.05:5.4, samples=300, smooth] (\x, {
    ...
    plot[domain=0:5.4, samples=300, smooth]
      ...
      % Flecha anotada
      \draw[->, red!50!black, thin] (3.0, -1.1) -- (2.2, {sin(deg(2*3.14159*0.5*2.2))-0.08});
      \node[font=\tiny, red!60!black, anchor=north] at (3.1,-1.1)
        {alias: $0{,}5\,\text{Hz}$};

      % Leyenda
      \draw[gray!55, thick]  (3.6, 1.35) -- (4.1, 1.35)
        node[right, font=\tiny, gray] {original};
      \draw[red!70!black, thick] (3.6, 1.05) -- (4.1, 1.05)
        node[right, font=\tiny, red!70!black] {alias};

    \end{scope}

  \end{tikzpicture}

  \caption{Reconstrucción de $x(t) = \sin(2\pi \cdot 0{,}9\,t)$
    ($f_{max} = 0{,}9\,\text{Hz}$, frecuencia de Nyquist $= 1{,}8\,\text{Hz}$)
    a partir de sus muestras (puntos).
    \textbf{(a)}~Con $f_s = 4\,\text{Hz}$ las muestras (azul)
    son suficientes para reconstruir fielmente la señal original (gris)
    mediante interpolación sinc~---~ambas curvas se superponen.
    \textbf{(b)}~Con $f_s = 1{,}4\,\text{Hz}$ las escasas muestras
    (rojo) son compatibles con una sinusoide de $0{,}5\,\text{Hz}$
    ($= f_s - f_{max}$), la frecuencia alias, en lugar de la original
    de $0{,}9\,\text{Hz}$: la información se ha perdido de forma
    irreversible.}\label{fig:reconstruccion-nyquist}
