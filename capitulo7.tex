...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=1.0, font=\small]
    ...
    %\node[left=4pt, font=\footnotesize\bfseries] at (-1.8,-3.1) {DISCO};
    ...
    % --- Eje ---
    \node[left=4pt, font=\footnotesize\bfseries] at (-1.8,-3.75) {EJE};
    \draw[gray] (-1.8,-3.75) -- (-0.15,-3.75);
    \draw[fill=gray!60, thick] (-0.15,-3.55) rectangle (0.15,-3.95);

    % --- Horquilla del sensor: abraza el disco ---
    % Brazo superior
    \draw[blue!70!black, line width=1.5pt] (0.35,-2.85) -- (1.1,-2.85);
    % Brazo inferior  
    \draw[green!60!black, line width=1.5pt] (0.35,-3.35) -- (1.1,-3.35);
    % Cuerpo vertical que une los brazos
    \draw[gray!60, line width=1pt] (1.1,-2.85) -- (1.1,-3.35);
  ...
  \end{tikzpicture}
  \caption[Encoder óptico incremental.]{%
    Encoder óptico incremental. Las ranuras del disco interrumpen
    periódicamente el haz entre fuente de luz y sensor, generando
    las señales en cuadratura A y~B que permiten determinar
    posición~$\theta$ y sentido de giro.}
  \label{fig:encoder}
\end{figure}
...
\begin{itemize}
    \item \emph{Incrementales}: Cuentan pulsos desde una posición inicial. Pierden la referencia si falla la alimentación.
    \item \emph{Absolutos}: El disco tiene un código binario único (típicamente código Gray) para cada posición. Conocen su ubicación exacta en todo momento.
\end{itemize}
