...
\begin{equation}\label{eq:fem_velocidad}
    e = B \cdot l \cdot v
\end{equation}
donde $l$ es la longitud total del conductor del bobinado expuesto al flujo, $B$ la intensidad del campo magnético y $v$ la velocidad del objeto. Como se observa en la Figura~\ref{fig:velocimetro}, el sensor genera una tensión alterna si el movimiento es vibratorio.
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[american, scale=1]
        ...
        % Salida
        \draw (2.2, 0.3) -- (2.2, -0.4) to[short, -o] (3, -0.4) node[right] {$V_{out} \propto \vec{v}(t)$};
    \end{tikzpicture}
...
\begin{marginfigure}[2em]
    ...
    \end{tikzpicture}
    \caption[Estructura de un geófono.]{Estructura interna de un geófono. La inercia de la masa suspendida permite medir la velocidad de vibración de la carcasa.}\label{fig:geofono}
\end{marginfigure}
