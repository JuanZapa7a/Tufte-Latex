...
\begin{marginfigure}[2em]
    ...
    \end{tikzpicture}
    \caption[Lazo termoeléctrico básico.]{Lazo termoeléctrico básico. La tensión de Seebeck $V_{AB}$ es proporcional a la diferencia de temperatura entre la unión de medida ($T_1$) y la unión de referencia ($T_2$).}\label{fig:seebeck_principio}
\end{marginfigure}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption[Curvas de respuesta para termopares estándar.]{Curvas de respuesta para termopares estándar. El tipo K es casi lineal y sensible, mientras que el tipo S (metales nobles) es menos sensible pero extremadamente estable a altas temperaturas.}\label{fig:curvas_termopares}
\end{marginfigure}
...
\begin{table}[ht]
    \centering
    \caption[Características de los termopares estándar según IEC.]{Características de los termopares estándar según IEC. Los valores de sensibilidad son promedios en sus rangos lineales.\protect\label{tab:tipos_termopares}}
    \setlength{\tabcolsep}{10pt}
    \begin{tabularx}{\linewidth}{M{2cm} X M{2cm} M{2cm}}
        \toprule
...
Es el más utilizado en la industria general debido a su amplio rango y buena resistencia a la oxidación. Sin embargo, no debe utilizarse en atmósferas reductoras sin protección, ya que el cromo puede sufrir un fenómeno de \emph{corrosión verde} que altera la calibración.
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \node[purple] at (axis cs:700, 65) {Tipo E};

            % K: ~41uV/C
            \addplot[ultra thick, red, domain=0:1200] {0.041*x};
            \node[red] at (axis cs:1000, 48) {Tipo K};

            % S: ~10uV/C (más curvado)
            \addplot[ultra thick, blue, domain=0:1200] {0.008*x + 0.000004*x^2};
            \node[blue] at (axis cs:1000, 18) {Tipo S};
        \end{axis}
    \end{tikzpicture}
    \caption[Comparativa de salida de tensión entre termopares.]{Comparativa de salida de tensión. El Tipo E ofrece la mayor relación señal-ruido, mientras que el Tipo S se reserva para aplicaciones donde la estabilidad es prioritaria sobre la amplitud de señal.}\label{fig:comparativa_mv}
\end{marginfigure}
...
Se utilizan exclusivamente en laboratorios de calibración y procesos de fundición de vidrio o acero. Su elevado coste se justifica por su inmunidad a la contaminación química y su capacidad de mantener la precisión a temperaturas extremas, aunque, como muestra la Figura~\ref{fig:comparativa_mv}, su sensibilidad de salida sea muy inferior a la de otros tipos.
...
\begin{marginfigure}[0em]
    ...
    \begin{tikzpicture}[american, scale=1]
        ...
        \node[blue, align=center] at (3.5, 0.6) {Sensor \\ $T_{ref}$};
    ...
    \end{tikzpicture}
    \caption[Esquema de un bloque de compensación de unión fría.]{Esquema de un bloque de compensación. Un sensor auxiliar (NTC o RTD) mide la temperatura de los terminales de cobre para permitir la corrección por software o hardware.}\label{fig:bloque_isotermico}
\end{marginfigure}
...
\begin{marginfigure}[3em]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \node[red, anchor=west] at (axis cs:1.2, 0.4) {$f_c$};
        \end{axis}
    \end{tikzpicture}
...
\marginnote[2em]{\emph{Temperatura de Curie:} La temperatura de curie de un material piezoeléctrico es la temperatura a la que el material pierde sus propiedades dipolares permanentemente. En el cuarzo es de \qty{573}{\degree C}, mientras que en cerámicas PZT ronda los \qty{300}{\degree C}.
}

\paragraph{Acondicionamiento: El Amplificador de Carga}
Si conectamos el sensor a un amplificador de tensión convencional, la capacidad parásita del cable ($C_c$) actúa como un divisor de carga en paralelo, haciendo que la tensión medida dependa críticamente de la longitud del cable ($V = q / (C_s + C_c)$). 

\begin{marginfigure}
    \centering
    \shorthandoff{>}
    \begin{tikzpicture}[american, scale=1]
        \ctikzset{resistors/scale=0.6, capacitors/scale=0.6, sources/scale=0.6, amplifiers/scale=1, amplifiers/fill=orange!5}
        % Sensor (Modelo fuente de carga)
        \draw (0,0) to[isource, l=$i_{in}$] (0,2) -- (0.7,2);
        \draw (0.7,2) to[C, l=$C_s$,*-*] (0.7,0);
        \draw (0,0) -- (1.6,0);
        % Cable
        \draw (0.7,2) -- (1.6,2) node[circ]{} to[C, l=$C_c$,*-*] (1.6,0) node[ground]{};
        % Amp
        \draw (2.5,2) node[op amp, anchor=-] (oa) {};
        \draw (1.6,2) -- (oa.-);
        \draw (oa.+) -- (oa.+ |- 0,0) node[ground]{};
        % Feedback
        \draw (oa.-) node[circ]{} -- ++(0,0.8) coordinate (top) to[C, l=$C_f$] (top -| oa.out) -- (oa.out) node[circ]{};
        \draw (oa.out) to[short, -o] ++(0.3,0) node[above] {$V_{out}$};
        % Tierra virtual highlight
        \node[red, align=center, font=\tiny] at (2, 2) {Tierra\\Virtual};
        %\draw[red, thick, <-] (2.2, 1.6) -- (2.2, 2.0);
    \end{tikzpicture}
    \caption[Efecto de la tierra virtual en el cable.]{Configuración de amplificador de carga. Al mantener el nodo inversor a potencial de masa (tierra virtual), la capacidad del cable $C_c$ no almacena carga ($q=C \cdot 0 = 0$), eliminando su influencia en la medida.}\label{fig:tierra_virtual_carga}
\end{marginfigure}

Para solucionar esto, se emplea el \emph{Amplificador de Carga} analizado en el Capítulo~\ref{cap:opamps}. Como se muestra en la Figura~\ref{fig:tierra_virtual_carga}, el operacional se conecta el sensor a la entrada inversora y mantiene el nodo inversor a masa (tierra virtual). Gracias a la tierra virtual, el potencial del conductor central del cable se mantiene en cero, anulando la carga acumulada en $C_c$ y forzando a que toda la carga generada por el sensor se transfiera íntegramente al condensador de realimentación $C_f$. Así, la tensión de salida depende exclusivamente del componente interno $C_f$:
\begin{equation}
...
Un ejemplo de esto puede verse de la siguiente manera: La sensibilidad se define como $S = \Delta V / \Delta q$. Veamos el caso de utilizar un amplificador de tensión convencional frente a un amplificador de carga:
    \begin{itemize}
        \item \emph{Amplificador de Tensión}: La tensión se rige por $V = q / (C_s + C_c)$. La sensibilidad $S_v$ depende críticamente del cable:
        \[ S_v = \frac{1}{C_s + C_c} \]
        Para $C_s = \qty{100}{pF}$ y $q = \qty{100}{pC}$, al pasar de \qty{1}{m} ($C_c \approx \qty{100}{pF}$) a \qty{10}{m} ($\qty{1000}{pF}$), la señal cae de \qty{0.5}{V} a \qty{0.09}{V}.
        \item \emph{Amplificador de Carga}: Debido a la tierra virtual ($V_- \approx 0$), el cable no almacena carga ($q_{cable} = C_c \cdot 0 = 0$). Toda la carga fluye a $C_f$:
        \[ S_q = -\frac{1}{C_f} \quad \implies \quad V_{out} = -\frac{q}{C_f} \]
        Con $C_f = \qty{100}{pF}$, la salida es \qty{-1}{V} constante, sin importar la longitud del cable.
    \end{itemize}

\marginnote{\textbf{Sensores Piezoeléctricos: Acelerómetros:} 
...
\begin{marginfigure}[0em]
    ...
    \begin{tikzpicture}[scale=1]
        % Unión PN
        \draw[fill=red!15] (0,0.5) rectangle (3,1); \node[red!70!black, font=\scriptsize] at (2.8,0.85) {Capa P};
        \draw[fill=blue!15] (0,0) rectangle (3,0.5); \node[blue!70!black, font=\scriptsize] at (0.3,0.15) {Capa N};
        
        % Pares electrón-hueco y su movimiento
        \foreach \xpos [count=\i] in {0.7, 1.5, 2.3} {
            % Hueco (+) hacia capa P
            \node[circle, fill=red, inner sep=0.5pt, font=\tiny, text=white] (h\i) at (\xpos, 0.6) {+};
            \draw[-stealth, red, very thin] (h\i.north) -- ++(0, 0.2);
            
            % Electrón (-) hacia capa N
            \node[circle, fill=blue, inner sep=0.5pt, font=\tiny, text=white] (e\i) at (\xpos+0.2, 0.4) {-};
            \draw[-stealth, blue, very thin] (e\i.south) -- ++(0, -0.2);
        }
        \node[align=center, font=\tiny] at (0.35, 0.5) {Pares\\$e^-/h^+$};
        ...
        % Circuito
        \draw (0.2,0.75) node[circ]{} -- (-0.6,0.75) -- (-0.6,-1) coordinate (bot_left);
        \draw (2.8,0.25) node[circ]{} -- (3.6,0.25) -- (3.6,-1) coordinate (bot_right);
        
        % Sentido de la corriente Iph (Sale de P hacia N)
        \draw[->, >=latex, blue!70!black, thick] (-0.6,-0.1) -- (-0.6,-0.7) node[left] {$I_{ph}$};
        \draw[->, >=latex, blue!70!black, thick] (3.6,-0.7) -- (3.6,-0.1);
        ...
        \node[blue!70!black, align=center, font=\scriptsize] at (1.5, -1.5) {Modo de medida\\(Baja impedancia)};
    \end{tikzpicture}
...
\marginnote[0em]{\emph{Respuesta Espectral:} La sensibilidad no es uniforme para todos los colores. Los sensores de silicio tienen su pico de respuesta en el infrarrojo cercano (aprox. \qty{900}{nm}). Para medir iluminancia visible, se requieren filtros ópticos que ajusten la respuesta a la curva fotópica del ojo humano.
}
...
\begin{equation}\label{eq:fem_velocidad}
    e = B \cdot l \cdot v
\end{equation}
donde $l$ es la longitud total del conductor del bobinado expuesto al flujo, $B$ la intensidad del campo magnético y $v$ la velocidad del objeto. Como se observa en la Figura~\ref{fig:sensores_electrodinamicos}, el sensor genera una tensión alterna si el movimiento es vibratorio.
%explicado en mas detalle.
Cuando el objeto se mueve a velocidad constante, la tensión de salida es continua. Sin embargo, si el objeto se mueve a velocidad variable, la tensión de salida será una señal alterna cuya frecuencia y amplitud dependen de la velocidad instantánea del objeto.
...
\begin{marginfigure}
    \centering
    \shorthandoff{>}
    \begin{tikzpicture}[american, scale=0.95]
        % --- PARTE (a): Transductor electrodinámico ---
        \begin{scope}[xshift=-1.7cm]
            % Imán fijo
            \draw[thick, fill=red!20] (0,1) rectangle (0.8,2);
            \draw[thick, fill=blue!20] (0,0) rectangle (0.8,1);
            \node[red] at (0.4, 1.5) {\emph{N}};
            \node[blue] at (0.4, 0.5) {\emph{S}};
            
            % Líneas de campo B
            \foreach \y in {0.6, 1, 1.4} {
                \draw[green!60!black, ->, dashed] (0.9, \y) -- (2.3, \y);
            }
            \node[green!60!black] at (1.6, 1.7) {$\vec{B}$};
            ...
            % Salida
            \draw (2.2, 0.3) -- (2.2, -0.4) node[ocirc, scale=0.6] {} node[below] {$V_{out} \propto \vec{v}(t)$};
            \node at (1.2, -1.6) {(a) Transductor lineal};
        \end{scope}
        ...
        \begin{scope}[ xshift=1.5cm]
            ...
            \node[gray] at (1.25, 3.7) {Carcasa (Unida al suelo)};

            % Imán central (Fijo a la carcasa)
            \draw[fill=red!20] (0.9, 1.2) rectangle (1.6, 1.9);
            \draw[fill=blue!20] (0.9, 0.5) rectangle (1.6, 1.2);
            \node[red] at (1.25, 1.55) {N};
            \node[blue] at (1.25, 0.85) {S};

            % Bobina (Suspendida)
            \draw[ultra thick, orange!80!black] (0.6, 0.8) rectangle (1.9, 2.3);
            \node[orange!80!black, rotate=90] at (0.4, 1.55) {Bobina (Masa)};

            % Muelles (Superior e inferior)
            \draw[decoration={zigzag,amplitude=2pt, segment length=4pt}, decorate, blue!60] (1.25, 3.5) -- (1.25, 2.3);
            \draw[decoration={zigzag,amplitude=2pt, segment length=4pt}, decorate, blue!60] (1.25, 0.5) -- (1.25, 0);
            \node[blue!60] at (1.6, 3) {$k$};

            % Vibración
            \draw[<->, thick, gray] (1.25, -0.4) -- (1.25, -1) node[below] {Vibración};
            \node at (1.25, -1.6) {(b) Estructura geófono};
        \end{scope}
    \end{tikzpicture}
    \caption[Sensores electrodinámicos.]{Sensores electrodinámicos: (a) Principio de funcionamiento de un transductor de velocidad lineal. (b) Estructura interna de un geófono, donde la inercia de la masa suspendida permite medir vibraciones terrestres.}\label{fig:sensores_electrodinamicos}
\end{marginfigure}

\marginnote{\textbf{Sensores Electrodinámicos: Limitación en DC:} Al igual que los piezoeléctricos, los sensores electrodinámicos son incapaces de realizar medidas estáticas. Si la velocidad es nula, el flujo magnético no cambia y la tensión de salida cae a cero, independientemente de la posición del sensor.
}

\section{Resumen}\label{sec:resumen_generadores}
...
\begin{itemize}
    \item \emph{Termopares:} Utilizan el efecto Seebeck para medir temperaturas. Son ideales para aplicaciones de alta temperatura y ambientes hostiles.
    \item \emph{Sensores Piezoeléctricos:} Basados en la generación de carga ante un esfuerzo mecánico. Son perfectos para medir vibraciones, impactos y aceleraciones.
    \item \emph{Sensores Fotovoltaicos:} Aprovechan el efecto fotovoltaico para medir la iluminancia. Son comunes en fotometría y aplicaciones de energía solar.
    \item \emph{Sensores Electrodinámicos:} Utilizan la inducción magnética para medir velocidades. Son esenciales en sismología y monitorización de maquinaria rotativa.
\end{itemize}
...
\begin{table}[ht]
    ...
    \caption[Comparativa de sensores generadores.]{Comparativa de sensores generadores. Nótese la diversidad de señales de salida (tensión, carga o corriente), lo que exige etapas de acondicionamiento específicas para cada tecnología.\protect\label{tab:resumen_generadores}}
