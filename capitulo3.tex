...
\fill[gray!60] (\x,\y) circle (4pt) node[white] {Si};
...
\emph{Conexión a 2 hilos: El error no compensado}

Es la configuración más sencilla pero la menos precisa. El equipo de medida detecta la suma de la resistencia del sensor ($R_s$) y la de los dos hilos de ida y vuelta.
...
\emph{Conexión a 3 hilos: Estándar Industrial}

Es la configuración más extendida en la industria. Se basa en el supuesto de que los tres hilos son idénticos ($R_{w1} \approx R_{w2} \approx R_{w3}$). Se utiliza típicamente en un **Puente de Wheatstone** para cancelar el efecto de los cables.
...
\emph{Conexión a 4 hilos: Medida Kelvin}
...
\sidenote{
    \textbf{Por qué funciona:} Al medir con un voltímetro de alta impedancia, la corriente por los hilos de "Sense" es despreciable ($< \unit{1}{nA}$). Según la ley de Ohm ($V=IR$), si la corriente es nula, la caída de tensión en los cables es nula, midiendo exclusivamente la caída en el sensor.
}
