...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[scale=1]
        ...
        % Leyenda
        \node[align=left, font=\tiny, anchor=north west] at (0, -0.5) {
...
Este salto energético genera un \textbf{par electrón-hueco}, transformando instantáneamente un estímulo óptico en portadores de carga eléctrica móviles. La importancia de este descubrimiento es incalculable: es el eslabón fundamental que permite convertir la luz en información, siendo la base de la imagen digital, las comunicaciones por fibra óptica y la energía solar fotovoltaica.
...
\begin{marginfigure}[2em]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            % Silicio
            \addplot[thick, blue, domain=300:1100, samples=50] {x/1100 * exp(-(x-1000)^2/50000)};
            \node[blue, font=\tiny] at (axis cs:700, 0.5) {Silicio (Si)};
            
            % InGaAs
            \addplot[thick, red, domain=800:1700, samples=50] {0.95 * exp(-(x-1550)^2/40000)};
            \node[red, font=\tiny] at (axis cs:1400, 0.7) {InGaAs};
...
\marginnote{
    \textbf{Energía del fotón:} La longitud de onda de corte ($\lambda_c$) es la máxima que puede detectar un sensor:
...
\marginnote[2em]{
    \textbf{Ruido de Disparo (\textit{Shot Noise}):} Es el ruido dominante en sensores ópticos. Se debe a la naturaleza discreta de los fotones y electrones. Su valor eficaz es $I_n = \sqrt{2qI\Delta f}$.
}
