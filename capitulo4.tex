...
\begin{table}[!ht]
    \centering
    \caption{Comparativa de modelos de sensibilidad. La variación de distancia ofrece una sensibilidad mucho mayor a costa de una respuesta no lineal.\protect\label{tab:sensibilidad_capacitiva}}
...
\begin{tikzpicture}[scale=1]
    ...
    \draw[dashed, gray] (2.5, 0) -- (2.5, 3.5)
        node[above, gray] {$x=0$};
\end{tikzpicture}
\caption{Sección transversal del LVDT. El primario está alimentado con una señal alterna y las secundarias están conectadas en oposición de serie. La varilla de accionamiento controla la posición del núcleo ferromagnético.}\label{fig:lvdt_estructura}
\end{marginfigure}
...
\begin{table}[!ht]
    \centering
    \caption{Comparativa de ventajas e inconvenientes del LVDT para el diseñador de sistemas de instrumentación.\protect\label{tab:lvdt_pro}}
    \small
...
\begin{table}[!ht]
    \centering
    \caption{Comparativa Hall / AMR / GMR para el diseño de instrumentación magnética. Los valores son aproximados. La variación de resistencia se expresa en \% de la resistencia de la capa libre. \protect\label{tab:hall_vs_mr}}
    \small
...
\begin{marginfigure}
    ...
    \end{tikzpicture}
    \caption{Acondicionamiento mediante conversión a frecuencia. El sensor forma parte de la red de realimentación de un oscilador.}\label{fig:oscilador_sensor}
\end{marginfigure}
...
\marginnote[-10em]{\textbf{Detección de dirección:} En el LVDT, si el núcleo se mueve a la derecha, $\phi=0^\circ \implies \cos(0)=1$ (V positiva). Si se mueve a la izquierda, $\phi=180^\circ \implies \cos(180)=-1$ (V negativa). Así recuperamos el signo del desplazamiento.}
...
\begin{marginfigure}[-2em]
    ...
    \begin{tikzpicture}[node distance=1.5cm, >=stealth]
        ...
        \draw [->] (ref) -- (mult);
    \end{tikzpicture}
    \caption{Diagrama de bloques de un demodulador síncrono. Esta estructura actúa como un filtro de banda estrecha extremadamente selectivo, eliminando cualquier ruido que no esté en fase con la referencia.}\label{fig:block_demod}
\end{marginfigure}
...
\section{Resumen de Errores en Sensores Reactivos}\label{sec:errores_reactivos}
