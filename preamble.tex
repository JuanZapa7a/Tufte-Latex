% Tufte-Latex/preamble.tex
% !TeX root = main.tex
% !TEX program = pdflatex


% Paquetes adicionales
\usepackage[spanish]{babel} % Idioma español
\usepackage{booktabs} % Tablas de calidad profesional
\usepackage{lipsum} % Texto de ejemplo
\usepackage{flafter} % Evita que tablas/figuras aparezcan antes de su referencia
\usepackage{float} % Control preciso de posición de floats
%\usepackage{microtype} % Mejoras tipográficas
%\UseMicrotypeSet[protrusion]{basicmath} % Ejemplo de configuración habitual
%\microtypesetup{disable=footnote}
\usepackage{csquotes} % Recomendado por biblatex con babel/polyglossia
\usepackage{pdfpages}
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}
\usepackage{tikz}
\usepackage[american]{circuitikz} % 'american' usa el símbolo zig-zag para R   
\usetikzlibrary{positioning, arrows.meta, babel} % Para flechas profesionales y compatibilidad con babel
\usepackage{xurl}

% mios
\usepackage{siunitx} % Para condicionales en el estilo de página
\usepackage{marginfix} % Para evitar problemas con márgenes y floats
\usepackage{tabularx}
\usepackage{cancel} % Para cancelar un termino en una ecuación
\usepackage{multirow} % Para tablas con celdas de varias líneas
\usepackage{listing} % Entorno flotante numerado para listados de código
\renewcommand{\listingname}{Listado} % Nombre del listado en los pies (por defecto, «Listing»)

% Las columnas X de tabularx son, por definición, p{ancho} (alineación
% superior). Redefinimos el hook oficial \tabularxcolumn para que usen
% m{ancho} (alineación centrada verticalmente) y además centrado horizontal.
\renewcommand\tabularxcolumn[1]{>{\centering\arraybackslash}m{#1}}
\newcolumntype{M}[1]{>{\centering\arraybackslash}m{#1}}
% Separación vertical entre filas: por defecto (arraystretch=1) las tablas
% con celdas de varias líneas quedan apelotonadas. Se aplica a todas las
% tablas del libro para mantener la misma respiración entre filas.
\renewcommand{\arraystretch}{1.3}
% Columna de ancho fijo con texto alineado a la izquierda pero, igual que M,
% centrada verticalmente: evita que la primera columna de una fila (a menudo
% de ancho fijo) quede pegada arriba mientras las columnas X vecinas se
% centran, lo que rompía la coherencia vertical de las tablas.
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}m{#1}}
%\usepackage{math} 

% \usepackage[most]{tcolorbox}

% % Definición del entorno Ejemplo
% \newtcolorbox[auto counter, number within=chapter]{example}[2][]{%
%     colback=blue!5!white, % Color de fondo
%     colframe=blue!75!black, % Color del borde
%     fonttitle=\bfseries,
%     title=Ejemplo~\thetcbcounter: #2,
%     #1
% }

% \usepackage{xcolor}

% % Comando para ejemplos en el margen
% \newcommand{\ejemploMargen}[2]{%
%   \marginnote{
%     \begin{small}
%       \begin{itshape}
%         \textcolor{blue!70!black}{\textbf{Ejemplo: #1}} \\
%         \scriptsize #2
%       \end{itshape}
%     \end{small}
%     \vspace{0.5cm}
%   }
% }

\definecolor{cellbg}{RGB}{180,185,230}      % lavender/blue for pixel array
\definecolor{registerbg}{RGB}{173,216,230}  % light blue for registers
\definecolor{ccdbg}{RGB}{200,200,200}       % gray for CCD chip body
\definecolor{ccdgreen}{RGB}{60,140,100}     % dark green border
\definecolor{ccdgold}{RGB}{220,180,100}     % gold dashed border


% Configuración de márgenes y diseño
\geometry{
paperwidth=210mm,
paperheight=297mm,
left=42pt,
top=72pt,
textwidth=320pt,
marginparsep=20pt,
marginparwidth=180pt,
textheight=700pt, % Ajusta la altura del texto 650 original
footskip=40pt,
}


% Fuente sans-serif moderna
\usepackage{cmbright} % Fuente matematica moderna
% o 
%\usepackage{tgpagella}
%\usepackage{sfmath} 

% Configuraciion de fuente sanserif para todo el documento
\renewcommand*\familydefault{\sfdefault} % Cambia la familia global
\renewcommand{\rmdefault}{\sfdefault} % Cambia la familia de fuente a sans-serif
 

\usepackage{amsmath, amsfonts, mathtools, amsthm, amssymb} % Paquetes de matemáticas
%\usepackage{mathrsfs} % Para fuentes matemáticas adicionales

% Definimos el estilo myplain
\fancypagestyle{myplain}{
  \fancyhf{} % limpiar encabezados y pies
  \ifthenelse{\boolean{@tufte@twoside}}%
    {% two-side
      \fancyhead[RO]{\rightmark}
      \fancyhead[LE]{\MakeUppercase{\thechapter. ~\leftmark}}
      \fancyheadoffset[LE,RO]{-\marginparsep+20pt}
      \fancyfoot[LE,RO]{\thepage}
      \fancyfootoffset[LE,RO]{-\marginparsep+20pt}
    }%
    {% one-side
      \fancyhead[RE,RO]{\smallcaps{\newlinetospace{\plaintitle}}\quad\thepage}
    }%
}

% Enganchamos en \mainmatter
\let\oldmainmatter\mainmatter
\renewcommand{\mainmatter}{%
  \oldmainmatter
  \pagestyle{myplain}%
}

% % También para los capítulos en el mainmatter
% \let\oldchapter\chapter
% \renewcommand{\chapter}{%
%   \cleardoublepage
%   \pagestyle{myplain}
%   \oldchapter
% }

%Configuracion aspecto del titulo del capítulo
\usepackage{titlesec,titletoc} % Carga el paquete helper
\usepackage{fmtcount} % Para formatear números en texto
\titleformat{\chapter}[display]
{\bfseries\Large\sf}
{\filleft
\MakeUppercase{\chaptertitlename}{~\Huge\NUMBERstring{chapter}}
}
{1.5ex}
{\LARGE\rmfamily
\titlerule
\vspace*{1.1ex}%
\filright}
[\vspace*{1.1ex}%
\titlerule]



\titlecontents{chapter}
  [0pt] % Indent
  {\bfseries\Large\sf} % Format
  {\addvspace{0.5ex}{\MakeUppercase{\chaptertitlename}}~\thecontentslabel  \addvspace{0.5ex}} % Numbering format
  {\addvspace{0.5ex}} % Above code
  {\hfill\contentspage} % Page number format
  [\addvspace{0.5ex}] % Space after



\setcounter{secnumdepth}{1} % Profundidad de numeración de secciones (1 para capítulos, 2 para secciones, etc.)
\setcounter{tocdepth}{1} % Profundidad del índice (1 para capítulos, 2 para secciones, etc.)

\newcommand{\monthyear}{\ifcase\month\or January\or February\or March\or April\or May\or June\or July\or August\or September\or October\or November\or December\fi\space\number\year} % A command to print the current month and year


\addto\captionsspanish{ % Traducciones de términos comunes
  \renewcommand{\tablename}{Tabla} % Nombre de tablas y no cuadros
  \renewcommand{\listtablename}{Índice de tablas}
}

\usepackage{graphicx}
\DeclareGraphicsRule{.pspdftex}{pdf}{.pdftex}{}
\usepackage{grffile} % Permite nombres de archivos con espacios y caracteres especiales
\graphicspath{{./} {imagenes/}{~/OneDrive/TexPlantillas}} % Ruta de búsqueda para imágenes

% Metadatos para PDF
\usepackage{hyperref}
\hypersetup{
  pdftitle={Ejemplo completo de tufte-book},
  pdfauthor={John Doe},
  pdfsubject={LaTeX, tufte-book},
  pdfkeywords={latex, tufte, ejemplo, libro},
  bookmarksnumbered=true,
  bookmarksopen=true,
  pdfstartview={FitH},
  colorlinks=true,
  linkcolor=MidnightBlue,
  citecolor=ForestGreen,
  urlcolor=RubineRed,
}
