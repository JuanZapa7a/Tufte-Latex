...
\begin{marginfigure}[0em]
    ...
    \begin{tikzpicture}[american, scale=1]
        ...
        \end{scope}
    \end{tikzpicture}
    \caption[Impedancias en el plano complejo para RL y RC serie.]{Representación en el plano complejo de las impedancias para circuitos RL y RC serie. La parte real representa la resistencia (disipativa) y la parte imaginaria la reactancia (almacenamiento).}\label{fig:impedancia_compleja}
\end{marginfigure}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \node[gray, anchor=south] at (axis cs:0.78, 1) {$\Delta \phi = \qty{90}{\degree}$};
        \end{axis}
    \end{tikzpicture}
...
%             \node at (1, -0.6) {\emph{(a) Almacenamiento en } $\vec{E}$};
%         \end{scope}
...
%             \node at (1, -0.6) {\emph{(b) Almacenamiento en } $\vec{B}$};
%         \end{scope}
%     \end{tikzpicture}
...
Los sensores capacitivos basan su funcionamiento en la modificación de uno o más parámetros de la ecuación fundamental del condensador de placas paralelas. Como resume la Figura~\ref{fig:modos_capacitivos}, existen tres estrategias de transducción: variar la distancia entre placas, variar su área de solapamiento o modificar el dieléctrico interpuesto, cada una con implicaciones distintas de sensibilidad y linealidad que se detallan a continuación:
...
\begin{table}[!ht]
    \centering
    \caption[Comparativa de modelos de sensibilidad capacitiva.]{Comparativa de modelos de sensibilidad. La variación de distancia ofrece una sensibilidad mucho mayor a costa de una respuesta no lineal.\protect\label{tab:sensibilidad_capacitiva}}
...
La diferencia de comportamiento entre ambos modos se aprecia claramente en la Figura~\ref{fig:curvas_sensibilidad}, donde la curva de variación de distancia crece de forma hiperbólica al aproximarse las placas, mientras que la de variación de área mantiene una pendiente constante en todo el rango.
...
En ingeniería de precisión, el problema de la no linealidad hiperbólica de los sensores de distancia se resuelve mediante el uso de un \emph{condensador diferencial} o configuración \textit{Push-Pull}. Este sistema, esquematizado en la Figura~\ref{fig:sensor_diferencial}, consta de tres placas: dos fijas y una central móvil que se desplaza entre ambas.
...
Si intentamos medir un sensor de \unit{1}{pF} mediante un cable de \unit{1}{m}, la capacidad del cable es \emph{100 veces superior} a la del sensor, actuando como un condensador en paralelo que, tal como se modela en la Figura~\ref{fig:cable_parasito}, enmascara la señal y degrada la sensibilidad.
%
\begin{marginfigure}[0em]
    \centering
    \shorthandoff{>}
...
Este modelo equivalente, representado en la Figura~\ref{fig:paralelo_capacitivo}, ilustra cómo la gran magnitud de $C_p$ enmascara la pequeña variación útil $\Delta C_s$ dentro de la capacidad total que ve la electrónica de acondicionamiento.
...
\begin{marginfigure}
    ...
    \end{tikzpicture}
    \caption[Modelo equivalente del sensor y el cable.]{Modelo equivalente del sensor y el cable. La gran magnitud de $C_p$ enmascara los pequeños cambios de $C_s$.}\label{fig:paralelo_capacitivo}
\end{marginfigure}
...
En esta técnica, tal como se detalla en la Figura~\ref{fig:guarda_activa}, se utiliza un seguidor de tensión (buffer) para alimentar el blindaje interno con exactamente la misma tensión que lleva el hilo de señal.
...
%     \end{tikzpicture}
%     \caption[Esquema de guarda activa.]{Esquema de guarda activa. El amplificador asegura que el potencial de la guarda sea idéntico al de la señal, anulando la corriente a través de $C_p$.}\label{fig:guarda_activa}
% \end{marginfigure}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[american, scale=1]
        ...
        \node[blue!50, font=\bfseries] at (3, .3) {BUFFER (Seguidor de tensión)};
        ...
        \node[gray!80, font=\bfseries] at (3, 3.1) {GUARDA (Malla)};
        ...
        % Nota aclaratoria
        \node[red, align=center] at (4.3, 1.7) {$V_{sig} \approx V_{guard}$  $\implies I_{Cp} \approx 0$};

    \end{tikzpicture}
    \caption[Esquema de guarda activa.]{Esquema de guarda activa. El buffer de ganancia unidad asegura que la malla del cable esté al mismo potencial que el hilo de señal ($V_{sig} \approx V_{guard}$), eliminando la corriente a través de $C_p$. Al no haber diferencia de tensión entre los terminales de $C_p$, esta capacidad no se carga ni descarga, eliminando su efecto en la medida del sensor.}\label{fig:guarda_activa}
\end{marginfigure}

\paragraph{Implementación en PCB: El Anillo de Guarda}
En el diseño de circuitos impresos (PCB), las corrientes de fuga por la superficie de la placa y las capacidades entre pistas también falsean la medida. Para evitarlo, se rodea la pista de alta impedancia del sensor con un \emph{anillo de guarda} conectado al mismo potencial, como se ilustra en la Figura~\ref{fig:anillo_guarda}.
...
\begin{marginfigure}
    ...
    \end{tikzpicture}
    \caption[Principio del touchscreen capacitivo.]{Principio del \textit{touchscreen} capacitivo. El cuerpo humano altera el campo eléctrico proyectado por la matriz de electrodos.}\label{fig:touch_capacitivo}
\end{marginfigure}
...
\begin{marginfigure}
  ...
  \begin{tikzpicture}[scale=1]
    ...
    \draw[<->, red, >=stealth] (0.3, 2.2) -- (0.3, 2.5)
          node[midway, left, red] {$g$};
    \draw[<->, red, >=stealth] (2.7, 2.2) -- (2.7, 2.5)
          node[midway, right, red] {$g$};
...
\begin{tikzpicture}[scale=1]
    ...
    \draw[dashed, gray] (2.5, 0) -- (2.5, 3.5)
        node[above, gray] {$x=0$};
\end{tikzpicture}
\caption[Sección transversal del LVDT.]{Sección transversal del LVDT. El primario está alimentado con una señal alterna y las secundarias están conectadas en oposición de serie. La varilla de accionamiento controla la posición del núcleo ferromagnético.}\label{fig:lvdt_estructura}
\end{marginfigure}

Como se muestra en la figura~\ref{fig:lvdt_estructura}, el LVDT consta de tres bobinas coaxiales: un \emph{primario} central excitado con una portadora $V_p \sin(\omega t)$ (típicamente entre \qty{1}{kHz} y \qty{10}{kHz}) y dos \emph{secundarios} (S1 y S2) situados simétricamente, conectados en oposición de serie:
...
El acoplamiento mutuo ($M$) entre el primario y cada secundario depende de la posición del núcleo ferromagnético:
...
\begin{align}
    V_{S1} &= A \cdot M_1(x) \cdot V_p \sin(\omega t + \phi) \\
    V_{S2} &= A \cdot M_2(x) \cdot V_p \sin(\omega t + \phi)
\end{align}

donde $A$ agrupa las constantes del circuito y $\phi$ el desfase introducido por la naturaleza inductiva. Al estar conectados en \emph{oposición de serie}, la salida total es la diferencia neta de ambos potenciales:

\begin{equation}
    V_{out}(x) = V_{S1} - V_{S2} = A \cdot V_p \sin(\omega t + \phi) \left[ M_1(x) - M_2(x) \right]
\end{equation}

Sustituyendo las expresiones de acoplamiento mutuo:

\begin{equation}
    V_{out}(x) = A \cdot V_p \sin(\omega t + \phi) \left[ (M_0 + k \cdot x) - (M_0 - k \cdot x) \right]
\end{equation}
...
\begin{equation}\label{eq:lvdt_lineal}
    V_{out}(x) = (2 \cdot A \cdot k) \cdot x \cdot V_p \sin(\omega t + \phi)
\end{equation}
...
\begin{enumerate}
    ...
    \item \emph{Información de dirección en la fase:} Para $x > 0$, $V_{out}$ está en fase con $V_p$ ($\phi = \ang{0}$); para $x < 0$, $V_{out}$ se invierte ($\phi = \ang{180}$). Un simple voltímetro de valor eficaz \emph{no puede distinguir} la dirección; se requiere \emph{demodulación síncrona}, tal como se ilustra en la comparativa de la Figura~\ref{fig:lvdt_curvas}.
\end{enumerate}
...
\begin{tikzpicture}
    ...
    xtick={-4,-2,0,2,4},
    ytick={-3,-2,-1,0,1,2,3},
    axis on top, % Asegura que los ejes queden sobre el fondo verde
    ...
    % Zona lineal (se dibuja primero para que actúe como fondo)
    \fill[green!10] (axis cs:-4,-2.4) rectangle (axis cs:4,2.4);

    % Salida con signo (se dibuja primero para que el azul se vea encima en x>0)
    \addplot[ultra thick, red, dashed, domain=-4.5:4.5, samples=100]
            {0.6*x};
    ...
    \node[green!50!black, font=\footnotesize, anchor=west] at (axis cs:0.5, 2.6){Zona lineal};
    \end{axis}
\end{tikzpicture}
\caption[Curvas de transferencia del LVDT.]{Característica de transferencia del LVDT. La curva azul ($|V_{out}|$) representa la magnitud de la salida, donde se pierde el signo de la posición. La línea roja ($V_{dem}$) es el resultado de la demodulación síncrona, que permite distinguir la dirección del desplazamiento mediante el signo de la tensión. El área verde delimita la región de operación lineal del sensor.}\label{fig:lvdt_curvas}
\end{marginfigure}
...
\begin{table}[!ht]
    \centering
    \caption[Ventajas e inconvenientes del LVDT.]{Comparativa de ventajas e inconvenientes del LVDT para el diseñador de sistemas de instrumentación.\protect\label{tab:lvdt_pro}}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[>=Stealth]
        % --- Target Conductor (Más ancho y alto) ---
        \draw[fill=gray!10, thick] (0,0) rectangle (4.8, 1.1);
        \node[anchor=south west, gray!80!black] at (0,-.5) {Target Metálico};

        % --- Coil (Cuerpo de la bobina más grande) ---
        % Coil former / Body
        \draw[fill=orange!5, orange!30, dashed] (1.2, 2.8) rectangle (3.6, 3.6);
        
        % Individual windings (Bottom row) - Aumentamos el radio de las espiras
        \foreach \x in {1.4, 1.8, 2.2, 2.6, 3.0, 3.4}
            \draw[thick, orange!80!black, fill=orange!20] (\x, 2.8) circle (3.2pt);
        % Individual windings (Top row)
        \foreach \x in {1.4, 1.8, 2.2, 2.6, 3.0, 3.4}
            \draw[thick, orange!80!black, fill=orange!20] (\x, 3.6) circle (3.2pt);
        
        \node[orange!80!black] at (2.4, 4.0) {Sonda (Bobina)};

        % --- Magnetic Flux (Primary) ---
        % Línea central
        \draw[blue, thick, dashed, ->] (2.4, 2.8) -- (2.4, 1.1);
        % Líneas laterales
        \draw[blue, thick, dashed, ->] (1.8, 2.8) .. controls (1.8, 1.6) and (0.7, 1.6) .. (0.7, 0.7);
        \draw[blue, thick, dashed, ->] (3.0, 2.8) .. controls (3.0, 1.6) and (4.1, 1.6) .. (4.1, 0.7);
        \node[blue] at (0.3, 2.0) {$\vec{B}_{exc}$};

        % --- Eddy Currents ---
        % Elipses más grandes para que se vean mejor los remolinos
        \draw[red, ultra thick, ->] (2.4, 0.6) ellipse (1.1cm and 0.22cm);
        \draw[red, thick, ->] (2.4, 0.6) ellipse (0.6cm and 0.12cm);
        \node[red, anchor=north] at (2.4, 0.4) {Corrientes inducidas};

        % --- Reaction Flux (Secondary) ---
        \draw[green!60!black, thick, ->] (1.5, 0.8) .. controls (1.5, 1.8) and (2.0, 2.4) .. (2.0, 2.7);
        \draw[green!60!black, thick, ->] (3.3, 0.8) .. controls (3.3, 1.8) and (2.8, 2.4) .. (2.8, 2.7);
        \node[green!60!black, anchor=west] at (3.5, 2.2) {$\vec{B}_{reac}$};

        % --- Cota delta (Ajustada al nuevo tamaño) ---
        \draw[<->, black, thin] (5.1, 1.1) -- (5.1, 2.8) node[midway, right ] {$\delta$};
    \end{tikzpicture}
    \caption[Principio de los sensores de corrientes de Foucault.]{Principio de funcionamiento de los sensores de corrientes de Foucault. El campo magnético primario ($\vec{B}_{exc}$) generado por la bobina induce corrientes de Foucault circulares en el target conductor. Estas generan, por la ley de Lenz, un campo de reacción ($\vec{B}_{reac}$) que se opone al original, alterando la impedancia compleja de la sonda según la distancia de separación $\delta$.}\label{fig:eddy_principio}
\end{marginfigure}
...
\end{equation}
%
donde $\rho$ es la resistividad del material y $f$ la frecuencia de excitación. La evolución de este parámetro para distintos materiales en función de la frecuencia se muestra en la Figura~\ref{fig:skin_depth}.

\marginnote{\textbf{Implicación de diseño:} La frecuencia de excitación determina la profundidad de análisis. A \qty{10}{kHz} en aluminio ($\rho = \qty{2.82e-8}{\Omega\cdot m}$), $\delta_{skin} \approx \qty{0.84}{mm}$. En cobre ($\rho = \qty{1.68e-8}{\Omega\cdot m}$), $\delta_{skin} \approx \qty{0.65}{mm}$. En acero ($\rho = \qty{1.5e-7}{\Omega\cdot m}$, $\mu_r \approx 1000$), $\delta_{skin} \approx \qty{0.06}{mm}$. Esto permite usar los sensores de Eddy Currents para \emph{END} (Ensayos No Destructivos): a baja frecuencia se detectan defectos internos; a alta frecuencia, solo superficiales.}
...
\begin{marginfigure}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      % Constantes calculadas para delta_skin (mm) = C / sqrt(f_kHz)
      % C = sqrt(rho / (pi * mu0 * mu_r * 1000)) * 1000
      % mu0 = 4*pi*1e-7
      % Aluminio: rho = 2.82e-8, mu_r = 1  => C_Al = 2.67
      \addplot[thick, blue, domain=1:500, samples=80] {2.67/sqrt(x)};
      \addlegendentry{Al ($\mu_r=1$)}

      % Cobre: rho = 1.68e-8, mu_r = 1 => C_Cu = 2.06
      \addplot[thick, orange, domain=1:500, samples=80] {2.06/sqrt(x)};
      \addlegendentry{Cu ($\mu_r=1$)}

      % Acero: rho = 1.5e-7, mu_r ≈ 1000 => C_Acero = 0.195
      \addplot[thick, red, dashed, domain=1:500, samples=80] {0.195/sqrt(x)};
      \addlegendentry{Acero ($\mu_r \approx 1000$)}

      % Linea guia
      \draw[gray, dotted] (axis cs:10, 0) -- (axis cs:10, 2.66)
           node[above] {\qty{10}{kHz}};
    \end{axis}
  \end{tikzpicture}
  \caption[Profundidad de penetración frente a frecuencia (efecto skin).]{Profundidad de penetración $\delta_{skin}$ frente a frecuencia de excitación para aluminio y acero. El acero, al ser ferromagnético ($\mu_r \gg 1$), confina las corrientes en una capa mucho más delgada.}\label{fig:skin_depth}
\end{marginfigure}
...
\begin{table}[ht]
    \centering
    \caption[Especificaciones típicas de sensores de corrientes de Foucault.]{Especificaciones típicas de los sensores de corrientes de Foucault industriales (familia Micro-Epsilon eddyNCDT).\protect\label{tab:eddy_specs}}
    \begin{tabularx}{\linewidth}{M{2cm} X}
        ...
        Inmunidad a contaminantes & Aceite, polvo, agua: \emph{total} \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\marginnote{\textbf{El signo de la carga:} Antes de este descubrimiento, no se podía distinguir si la corriente era un flujo de cargas positivas en una dirección o negativas en la opuesta. Al observar la polaridad de la tensión $V_H$, se confirmó definitivamente la naturaleza electrónica de la conducción metálica.}

Cuando una corriente $I$ fluye por un conductor situado en un campo magnético $\vec{B}$ perpendicular, los portadores experimentan la \emph{fuerza de Lorentz} ($\vec{F} = q\vec{v} \times \vec{B}$). Esta fuerza desvía los electrones (o huecos) hacia una de las caras laterales del material, acumulando carga hasta que el campo eléctrico transversal generado compensa exactamente la fuerza magnética. La geometría típica de un sensor de efecto Hall se muestra en la Figura~\ref{fig:hall_vectorial}. La tensión de Hall resultante es:
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[
        ...
        \def\W{4.0} % Ancho (x)
        \def\H{0.8} % Alto (y)
        \def\D{2.5} % Profundidad (z)

        % --- CUERPO DEL SEMICONDUCTOR (Caras traseras e inferior) ---
        \fill[cyan!5, draw=brown!40!black] (0,0,0) -- (\W,0,0) -- (\W,0,\D) -- (0,0,\D) -- cycle; % Base
        \fill[cyan!10, draw=brown!40!black] (0,0,0) -- (0,\H,0) -- (\W,\H,0) -- (\W,0,0) -- cycle; % Trasera

        % --- CAMPO MAGNÉTICO (B) - Parte inferior y atravesando ---
        \foreach \ix in {1.0, 2.0, 3.0} {
            \draw[green!60!black, thick] (\ix, -0.8, \D/2) -- (\ix, 0, \D/2); % Tramo inferior
            \draw[green!60!black, thin, dashed] (\ix, 0, \D/2) -- (\ix, \H, \D/2); % Tramo interno
        }

        % --- FLUJO DE PORTADORES (Superficie) ---
        % Dibujamos vectores en la superficie superior
        \foreach \iz in {0.6, 1.9} {
            \foreach \ix in {0.2, 1.5, 2.8} {
                \draw[purple!70, thick, ->] (\ix, \H, \iz) -- (\ix+0.8, \H, \iz); 
            }
        }
        \node[purple] at (2.0, \H+0.4, 0.2) {Corriente de huecos ($I$)};

        % --- CUERPO DEL SEMICONDUCTOR (Caras frontales y superior) ---
        \fill[red!20, opacity=0.4, draw=brown!80!black, thick] (0,0,\D) -- (\W,0,\D) -- (\W,\H,\D) -- (0,\H,\D) -- cycle; % Frontal
        \fill[cyan!15, opacity=0.3, draw=brown!80!black, thick] (0,\H,0) -- (\W,\H,0) -- (\W,\H,\D) -- (0,\H,\D) -- cycle; % Superior
        \draw[brown!80!black, thick] (\W,0,0) -- (\W,\H,0) -- (\W,\H,\D) -- (\W,0,\D) -- cycle; % Lateral derecha

        % --- CAMPO MAGNÉTICO (B) - Parte superior ---
        \foreach \ix in {1.0, 2.0, 3.0} {
            \draw[green!60!black, thick, ->] (\ix, \H, \D/2) -- (\ix, \H+1.4, \D/2);
        }
        \node[green!60!black, font=\footnotesize\bfseries] at (3.5, \H+1.2, \D/2) {$\vec{B}$};

        % --- ACUMULACIÓN DE CARGAS (Separación por Lorentz) ---
        \foreach \ix in {1.0, 2.0, 3.0} {
            \node[cargap] at (\ix, \H/2, \D) {$+$}; % Huecos al frente
            \node[cargan] at (\ix, \H/2, 0) {$-$};   % Electrones al fondo
        }
        \node[anchor=south west, font=\bfseries, gray!40!black] at (0, 0, \D) {Tipo P};

        % --- INDICACIÓN DE DIFERENCIA DE POTENCIAL (Arrow) ---
        % Líneas de referencia para V_H
        \draw[gray, thin] (\W, \H/2, \D) -- (\W+0.5, \H/2, \D);
        \draw[gray, thin] (\W, \H/2, 0) -- (\W+0.5, \H/2, 0);
        \draw[<->, red!50!black, thick] (\W+0.4, \H/2, \D) -- (\W+0.4, \H/2, 0) 
            node[midway, right, yshift=-3pt] {$V_H$};
        \node[red!50!black, align=left] at (\W+0.4, \H-1, \D/2) {Tensión\\transversal};

        % --- DIMENSIONES ---
        % Líneas de referencia y cota para d (espesor)
        \draw[gray, thin] (0, 0, \D) -- (-0.3, 0, \D);
        \draw[gray, thin] (0, \H, \D) -- (-0.3, \H, \D);
        \draw[<->, gray] (-0.2, 0, \D) -- (-0.2, \H, \D) node[midway, left] {$d$};

        % Líneas de referencia y cota para w (longitud)
        \draw[gray, thin] (0, 0, \D) -- (0, -0.5, \D);
        \draw[gray, thin] (\W, 0, \D) -- (\W, -0.5, \D);
        \draw[<->, gray] (0, -0.4, \D) -- (\W, -0.4, \D) node[midway, below] {$w$};

    \end{tikzpicture}
    \caption[Principio del efecto Hall.]{Principio del efecto Hall. El campo magnético $\vec{B}$ atraviesa verticalmente el material. La fuerza de Lorentz desvía los portadores mayoritarios (huecos en tipo P) hacia la cara posterior, estableciendo una diferencia de potencial transversal $V_H$.}\label{fig:hall_vectorial}
\end{marginfigure}
...
\marginnote{\textbf{Para el diseñador:} La elección entre Hall, AMR y GMR depende del rango de campo y la sensibilidad requerida. \emph{Hall}: campos fuertes (\qtyrange{0.1}{1}{\tesla}), bajo coste. \emph{AMR}: campos medios (\qty{1}{\micro\tesla} a \qty{1}{\milli\tesla}), buena linealidad. \emph{GMR}: campos débiles (\qty{<1}{\micro\tesla}), máxima sensibilidad pero no lineal.}%

Los \emph{sensores GMR} (Giant Magnetoresistance) alcanzan variaciones de hasta el \qty{80}{\%} mediante multicapas cuánticas ferromagnético/antiferro\-mag\-né\-tico (Fe/Cr). Son la base de los cabezales de lectura de discos duros y de los sensores de ángulo en los codificadores de precisión. La Tabla~\ref{tab:hall_vs_mr} resume las características de estos tres tipos de sensores para aplicaciones de instrumentación magnética.   

\marginnote{\textbf{Premio Nobel 2007:} Albert Fert y Peter Grünberg compartieron el galardón por el descubrimiento de la GMR de forma independiente. Este avance permitió que la densidad de almacenamiento en los discos duros aumentara de forma exponencial, reduciendo el tamaño de los cabezales de lectura y haciendo posible la era del \emph{Big Data} y la tecnología portátil actual.}
\clearpage

\begin{table}[!ht]
    \centering
    \caption[Comparativa Hall / AMR / GMR.]{Comparativa Hall / AMR / GMR para el diseño de instrumentación magnética. Los valores son aproximados. La variación de resistencia se expresa en \% de la resistencia de la capa libre. \protect\label{tab:hall_vs_mr}}
    \setlength{\tabcolsep}{4pt}
    \begin{tabularx}{\linewidth}{M{2cm} M{2cm} M{2cm} X X}
        \toprule
        \textbf{Tipo} & \textbf{Sensibilidad} & \textbf{Rango} & \textbf{Linealidad} & \textbf{Aplicación típica} \\
        \midrule
...
\paragraph{Puente de Maxwell} Se utiliza para medir una inductancia ($L_x, R_x$) desconocida comparándola con un condensador patrón ($C_s$). El esquema de medición se muestra en la Figura~\ref{fig:puente_maxwell}. Su gran ventaja es que las condiciones de equilibrio son independientes de la frecuencia.
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[american]
        ...
        % Rama superior izquierda (R1 || Cs) en paralelo - Simetría de diamante
        \draw (Va) -- ++(-0.3, 0.3) to[R, l=$R_1$] ++(1.5, 1.5) -- (0,3) coordinate (Vplus);
        \draw (Va) to[C, l_=$C_s$] (Vplus);
        \fill (Va) circle (2pt);
        \fill (Vplus) circle (2pt);

        \draw (0,0) to[L, l_=$L_x$, *-] (0.75,0.75) to[R, l_=$R_x$, -*] (1.5,1.5) coordinate (Vb);
        \draw (1.5,1.5) to[R, l_=$R_2$, *-*] (0,3);
        
        % Fuente de excitación conectada a la diagonal de alimentación
        \draw (Vplus) -- (2.2,3) to[vsourcesin, l=$v_{exc}$] (2.2,0) -- (Vminus);
        
        % Tensión de salida diferencial
        \draw[<->, red, thick] (Va) -- (Vb) node[midway, below] {$v_o$};
    \end{tikzpicture}
    \caption[Puente de Maxwell.]{Puente de Maxwell para la medida de inductancias. Al equilibrar el puente, se obtienen los valores de $L_x$ y $R_x$ a partir de componentes puramente resistivos y un condensador patrón $C_s$.}\label{fig:puente_maxwell}
\end{marginfigure}
...
\begin{equation}
    \frac{R_x + j\omega L_x}{R_3} = R_2 \left( \frac{1}{R_1} + j\omega C_s \right)
\end{equation}

Al igualar la parte real con la real, y la imaginaria con la imaginaria, se llega al resultado final:
\begin{equation}
    R_x = \frac{R_2 R_3}{R_1} \quad ; \quad L_x = R_2 R_3 C_s
\end{equation}

\marginnote{\textbf{Independencia de la frecuencia:} Observe que la frecuencia $\omega$ ha desaparecido de ambas ecuaciones. Esto significa que el equilibrio del puente de Maxwell no depende de la estabilidad del oscilador de excitación, lo que lo hace sumamente preciso para caracterizar bobinas con pérdidas resistivas significativas.}

\subsection{Circuitos Resonantes y Osciladores}

En aplicaciones donde la señal debe transmitirse a larga distancia o en entornos con mucho ruido de amplitud, se prefiere convertir la variación de $L$ o $C$ en una variación de \emph{frecuencia}. 

Al integrar el sensor en un circuito resonante $LC$, la frecuencia de oscilación viene dada por la fórmula de Thomson:
\begin{equation}
    f_r = \frac{1}{2\pi \sqrt{LC}}
\end{equation}
Cualquier cambio en la magnitud física que altere la capacidad del sensor provocará un desplazamiento de la frecuencia ($\Delta f$). Esta señal digital es intrínsecamente robusta y fácil de medir mediante el contador/temporizador de un microcontrolador sin necesidad de un ADC de alta precisión. La Figura~\ref{fig:oscilador_sensor} muestra un ejemplo de cómo integrar un sensor capacitivo en la red de realimentación de un oscilador para convertir su variación en una frecuencia proporcional a la magnitud medida.
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}[scale=1.0]
      \ctikzset{resistors/scale=0.6, capacitors/scale=0.6, inductors/scale=0.6}
        % Bloque Oscilador
        \draw[fill=blue!10, thick] (0,0) rectangle (2.5,1.5);
        \node at (1.25, 0.75) {Oscilador};
        
        % Sensor LC
        \draw (1.25, 0) -- (1.25, -0.5) node[circ,scale=0.6]{};
        \draw (0.5, -0.5) to[C, l=$C_s$] (0.5, -1.5) node[ground]{};
        \draw (2.0, -0.5) to[L, l=$L$] (2.0, -1.5) node[ground]{};
        \draw (0.5, -0.5) -- (2.0, -0.5);
        
        % Salida
        \draw[->, thick] (2.5, 0.75) -- (4, 0.75) node[right] {$f_{out} \propto \frac{1}{\sqrt{C_s}}$};
        
        % Estímulo
        \draw[->, ultra thick, red] (1.25, -2.2) -- (1.25, -1.7) node[below, pos=0] {Magnitud Física};
    \end{tikzpicture}
    \caption[Acondicionamiento mediante conversión a frecuencia.]{Acondicionamiento mediante conversión a frecuencia. El sensor forma parte de la red de realimentación de un oscilador.}\label{fig:oscilador_sensor}
\end{marginfigure}
...
\begin{equation}
    v_{mult}(t) = v_s(t) \cdot v_{ref}(t) = V_s V_r \sin(\omega t + \phi) \sin(\omega t)
\end{equation}
Aplicando la identidad trigonométrica $\sin A \sin B = \frac{1}{2} [\cos(A-B) - \cos(A+B)]$, obtenemos: 

\begin{equation}
    v_{mult}(t) = \frac{V_s V_r}{2} \left[ \cos(\phi) - \cos(2\omega t + \phi) \right]
\end{equation}
Como se muestra en la Figura~\ref{fig:block_demod}, tras pasar por un filtro paso bajo (Low Pass Filter, LPF), eliminamos el término de alta frecuencia ($2\omega$), obteniendo una tensión de continua proporcional al coseno del desfase:
\begin{equation}
    V_{out, DC} = \frac{V_s V_r}{2} \cos(\phi)
\end{equation}

\marginnote[-10em]{\emph{Detección de dirección:} En el LVDT, si el núcleo se mueve a la derecha, $\phi=0^\circ \implies \cos(0)=1$ (V positiva). Si se mueve a la izquierda, $\phi=180^\circ \implies \cos(180)=-1$ (V negativa). Así recuperamos el signo del desplazamiento.}

\begin{marginfigure}[-2em]
    \centering
    \shorthandoff{>}
    \begin{tikzpicture}[node distance=1.5cm, >=stealth]
        \tikzstyle{block} = [draw, rectangle, minimum height=0.8cm, minimum width=1.2cm]
        \tikzstyle{sum} = [draw, circle, inner sep=0pt, minimum size=0.5cm]

        \node (in) {$v_s(t)$};
        \node [sum, right of=in] (mult) {$\times$};
        \node [block, right of=mult, node distance=2cm] (lpf) {LPF};
        \node [right of=lpf] (out) {$V_{DC}$};
        \node [below of=mult, node distance=1.2cm] (ref) {$v_{ref}(t)$};
        ...
        \draw [->] (ref) -- (mult);
    \end{tikzpicture}
    \caption[Diagrama de bloques de un demodulador síncrono.]{Diagrama de bloques de un demodulador síncrono. Esta estructura actúa como un filtro de banda estrecha extremadamente selectivo, eliminando cualquier ruido que no esté en fase con la referencia.}\label{fig:block_demod}
\end{marginfigure}

\section{Resumen y Comparativa de Sensores Reactivos}\label{sec:resumen_reactivos}

Para concluir el estudio de los sensores reactivos, la Tabla~\ref{tab:comparativa_final} resume las tecnologías analizadas. La elección dependerá de la precisión requerida y las condiciones ambientales. 
  Si se busca la máxima resolución y el entorno es controlado, los sensores capacitivos son la mejor opción. Para aplicaciones industriales robustas, los LVDT son la referencia. En entornos sucios o con alta velocidad, los Eddy Currents son insuperables. Para aplicaciones de bajo coste o integración en ICs, los sensores de efecto Hall y AMR son la elección natural.
...
\begin{table}[!ht]
    ...
    \caption[Comparativa general de tecnologías reactivas.]{Comparativa general de tecnologías reactivas. Se resaltan las ventajas y limitaciones de cada tecnología. Destacamos que la elección dependerá de la precisión requerida y las condiciones ambientales.\protect\label{tab:comparativa_final}}
    \setlength{\tabcolsep}{3pt}
    \begin{tabularx}{\linewidth}{M{2cm} M{2cm} X X}
        \toprule
        \textbf{Tipo} & \textbf{Variable} & \textbf{Ventaja clave} & \textbf{Limitación} \\
        \midrule
...
\marginnote{\textbf{Conclusión:} Mientras que los sensores resistivos son la solución de bajo coste, los reactivos son la base de la \textit{instrumentación de precisión}. El dominio de la fase y el acondicionamiento en alterna permite al ingeniero diseñar sistemas con una relación señal-ruido inalcanzable para la electrónica puramente resistiva.}

\section{Resumen}\label{sec:errores_reactivos}
...
Como se ha estudiado, los errores en la cadena de medida reactiva pueden clasificarse en sistemáticos y aleatorios. \marginnote{\textbf{Prioridades de diseño:} En sensores capacitivos de muy baja capacidad (\unit{pF}), el error dominante suele ser la \textbf{capacidad parásita} del cableado. En sensores inductivos como el LVDT, el error principal proviene de la \textbf{tensión residual de cuadratura} en el punto nulo.} En ambos casos, la mitigación de estos errores es fundamental para alcanzar la resolución teórica del sensor. La Tabla~\ref{tab:resumen_errores_reactivos} resume las fuentes de error más relevantes, su manifestación en la medida y las técnicas de mitigación recomendadas.

\subsection{El Presupuesto de Errores (\textit{Error Budget})}
El \textit{error budget} es una herramienta esencial para el ingeniero de instrumentación, que permite cuantificar y priorizar las fuentes de error en un sistema de medida. Para un sensor reactivo, el error total ($E_{total}$) se puede expresar como la raíz cuadrada de la suma de los cuadrados de los errores individuales:
\begin{equation}
    E_{total} = \sqrt{E_1^2 + E_2^2 + \ldots + E_n^2}
\end{equation}
...
\begin{table}[!ht]
    \centering
    \caption[Clasificación y mitigación de errores en sistemas de medida reactivos.]{Clasificación y mitigación de errores en sistemas de medida reactivos.\protect\label{tab:resumen_errores_reactivos}}
    \setlength{\tabcolsep}{2pt} % Reducimos el espacio entre columnas
    \begin{tabularx}{\linewidth}{M{2cm} X X}
        \toprule
        \textbf{Fuente de Error} & \textbf{Manifestación Medida} & \textbf{Técnica de Mitigación} \\
        \midrule
        Capacidades parásitas & Atenuación masiva de la sensibilidad relativa ($\Delta C/C$). & Guarda activa (\textit{driven shield}) y anillos de guarda. \\
