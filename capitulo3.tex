...
\begin{marginfigure}[2em]
    \centering
    \shorthandoff{>}
    \begin{tikzpicture}[american, scale=0.7, font=\tiny]
...
\begin{marginfigure}[2em]
    ...
    \begin{tikzpicture}[american, scale=0.8, font=\scriptsize]
        % Sensor
        \draw[thick, blue] (2,0.5) to[R, l=$R_s$] (2,-0.5);
        % Hilos de corriente (Force)
        \draw (0,1) node[left] {$I_{source}$} to[R, l=$R_w$] (2,1) -- (2,0.5);
        \draw (0,-1) node[left] {Return} to[R, l=$R_w$] (2,-1) -- (2,-0.5);
        % Hilos de tensión (Sense)
        \draw (2,0.5) -- (1,0.5) to[R, l=$R_w$] (0.5,0.5) to[short, -o] (0,0.5) node[left] {$V_{+}$};
        \draw (2,-0.5) -- (1,-0.5) to[R, l=$R_w$] (0.5,-0.5) to[short, -o] (0,-0.5) node[left] {$V_{-}$};
    \end{tikzpicture}
